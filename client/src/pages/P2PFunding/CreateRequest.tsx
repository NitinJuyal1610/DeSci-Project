...
const CreateRequest = () => {
  ...
  const validate = () => {
    ...
    if (
      userBalance &&
      toBigNumber(totalAmountRequired.toString()).gt(userBalance)
    ) {
      openModal("warningModal");
      setModalData((prev) => ({
        ...prev,
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    console.log("submitting, ", process.env.REACT_APP_WEB3_STORAGE_API);
    if (
      validate() &&
      process.env.REACT_APP_WEB3_STORAGE_API &&
      client &&
      account
    ) {
      try {
              ...
              {formData.timeline.map((data, index) => (
                ...
                >
                  ...
                  <div className="cross-icon">
                    {index !== 0 && (
                      <CrossIcon onClick={() => removeTimeline(index)} />
                    )}
                  </div>
                </div>
              ))}

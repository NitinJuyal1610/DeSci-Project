...
const SubmitMilestoneModal: React.FC = () => {
  const { modalData, closeModal, openModal, setModalData } =
  ...
  const proposalFound = useMemo(
    () =>
      proposals
        ? proposals.find(
            (proposal) => proposal.id.toString() === modalData.data
          )
        : undefined,
    ...
    try {
      ...
      if (isFinalMilestone) {
        ...
        const jsonObj = JSON.stringify({
          ...
          image: `ipfs://${proposalFound.pictureCid}`,
          proposal: proposalFound.researchCid,
